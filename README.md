...
Its peer layer implements the openly published [RFC-F0690-v3: NOWFLOCK Peer Coordination Protocol](rfc/NOWFLOCK_STANDARD.md). The specification text is licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/): anyone may read, copy, adapt, redistribute, and implement FNOW/3, including commercially. Copied or adapted specification text keeps attribution and marks changes; compatible implementations owe no protocol fee. That openness covers the RFC, not separately copyrighted firmware, and it still does not turn radio capability into authorization. Its peer roles, channel claims, sighting tiles, target frames, and opt-in wardrive snapshots can also coordinate drone-carried wardriving or distributed Flock-camera hunts; the RFC coordinates radios, not flight controls or detector logic. Just saying, folks. The packet escaped. The warranty paperwork did not.
